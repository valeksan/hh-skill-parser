...
Каждый вызов CLI автоматически дописывает ход запуска и фатальные ошибки в
`logs/hh-skill-parser-YYYY-MM-DD.log` рядом с выбранным SQLite-файлом; путь
выводится в `stderr` при старте. Дополнительные перенаправления через `tee` не
нужны. Ошибки отдельных поисковых запросов и карточек по-прежнему сохраняются
в таблице `collection_errors`.
...
### Условия выдачи OAuth и ограничения API HH

По опубликованным условиям разработчиком может быть любой пользователь
интернета, принявший условия API; для регистрации приложения нужен аккаунт HH.
Это не означает гарантированную выдачу ключа: HH вправе отказать в регистрации,
ограничить отдельные функции или приостановить уже выданный ключ. Отказ и
приостановку можно обжаловать повторной заявкой в `dev.hh.ru/admin`; правила
указывают срок рассмотрения до 15 календарных дней. См. [условия API HH,
разделы 1–2](https://dev.hh.ru/admin/developer_agreement).

OAuth подтверждает доступ приложения к данным или действиям от имени
авторизованного пользователя, но не даёт самостоятельного права собирать,
хранить, публиковать или перепродавать данные HH. Для пользовательского OAuth
HH использует перенаправление на `redirect_uri`, временный код, PKCE и обмен на
пару `access_token`/`refresh_token`; у токена доступа есть срок жизни, а токен
обновления одноразовый. Секрет приложения и оба токена нельзя включать в код,
Git, логи или публичные выгрузки. См. [официальную документацию OAuth
HH](https://api.hh.ru/openapi/redoc).

При регистрации честно укажите фактическую цель и функции приложения. HH
разрешает использовать материалы в тематике сайта, включая информацию о рынке
труда, но может отказать, если видит использование не для подбора и
трудоустройства. Для работодателей и кадровых агентств правила отдельно
ограничивают использование привлечением и трудоустройством кандидатов. См.
[условия API HH, пункты 1.5–1.6 и 3.3](https://dev.hh.ru/admin/developer_agreement).

Особенно важно для сервиса аналитики: не создавайте посредством OAuth/API
публичную копию HH, поисковик вакансий, выгружаемую базу или API с сырыми
карточками. Условия запрещают формировать другую базу для предоставления данных
из неё третьим лицам и передавать полученные данные сторонним сервисам. Для
публичного продукта допустимость агрегированной обезличенной аналитики лучше
согласовать с HH письменно до запуска. См. [условия API HH, пункты 4.3 и
4.6](https://dev.hh.ru/admin/developer_agreement).

Если приложение предназначено для соискателей, его форма регистрации должна
информировать пользователя о соглашении HH по содействию в трудоустройстве и
давать возможность ознакомиться с ним. Это отдельное требование условий API,
не заменяемое OAuth-экраном. См. [условия API HH, пункт
1.4](https://dev.hh.ru/admin/developer_agreement).
...
### Промежуточный DA-срез

В [research/2026-07-31](research/2026-07-31/README.md) сохранён первый
версионируемый срез из 999 релевантных latest-вакансий: агрегированные частоты
навыков, публикаций, совместной встречаемости и два графика. В нём нет текстов
объявлений, raw payload, идентификаторов вакансий, контактных или точных
адресных данных.
Скрипты для перерисовки графиков находятся в `scripts/` и используют только
агрегированные таблицы research-среза.

Первичный вывод: 294 из 999 вакансий (29,4%) содержат навык «воинский учёт»;
следующие по частоте — бронирование граждан в запасе (56) и взаимодействие с
военным комиссариатом (40). Почти все публикации среза относятся к июлю 2026
года (997 из 999), поэтому график не следует интерпретировать как долгосрочный
тренд. HTML-источник не дал структурированных полей зарплаты, опыта, формата
работы и нормализованной географии — сравнения по ним в этом срезе делать
нельзя.
