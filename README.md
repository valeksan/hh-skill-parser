...
Пример сгенерированного графика:

![Пример графика навыков](hh_skills_bar_chart.example.png)
...
Список навыков для анализа. Строки, начинающиеся с `#`, игнорируются. Одиночная строка задаёт один навык.

Для тождественных вариантов используйте alias-группу: первым указывается каноническое название, затем варианты через `|`. В итоговом CSV попадёт только каноническое название; повторные alias одной вакансии считаются один раз.
...
Каждая непустая строка из `queries.txt` — самостоятельная поисковая фраза. Скрипт удаляет только внешние кавычки, если они были добавлены вручную, и передаёт запрос HH в двойных кавычках для точного поиска.

Запрос не разбивается на слова:

```text
queries.txt: Front-end developer
HH text:    "Front-end developer"
```

Строки с ручными кавычками совместимы и не получают двойное экранирование:
