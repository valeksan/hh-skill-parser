...
def export_marts(
    ...
    marts = {
        ...
        "missing_data": "WITH fields(field) AS (VALUES ('published_at'),('geography'),('area_id'),('area_name'),('federal_district'),('federal_subject'),('locality'),('employer'),('employer_id'),('employer_name'),('employer_type'),('salary_from'),('salary_to'),('salary_currency'),('salary_gross'),('salary_frequency'),('description'),('key_skills'),('industries'),('experience'),('employment'),('schedule'),('work_format')) SELECT fields.field, COALESCE(json_extract(selected.completeness_json, '$.fields.' || fields.field || '.missing_reason'), CASE WHEN json_extract(selected.completeness_json, '$.fields.' || fields.field || '.present') = 1 THEN 'present' ELSE 'not_recorded' END) AS missing_reason, COUNT(*) AS vacancy_count FROM selected CROSS JOIN fields GROUP BY 1,2 ORDER BY 1,2",
