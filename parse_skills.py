...
DEFAULT_PAGE_DELAY_MIN = 8.0
DEFAULT_PAGE_DELAY_MAX = 15.0
DEFAULT_VACANCY_DELAY_MIN = 3.0
DEFAULT_VACANCY_DELAY_MAX = 6.0
...
def cli_parse(argv: list[str] | None = None):
    ...
    Returns:
        argparse.Namespace:
            ...
            csv_output (str): Целевой CSV-файл со статистикой
            progress_file (str): Файл сохранения прогресса
            vacancies_limit (int):
    ...
    parser.add_argument(
        ...
        help="CSV-файл для вывода статистики (%(default)s)",
    )

    parser.add_argument(
        "--progress-file",
        default="progress.json",
        help="Файл сохранения прогресса (%(default)s)",
    )
...
def generate_chart_from_csv(
    ...
    if pyplot is None:
        raise RuntimeError(
            "Для команды chart установите поддержку графиков: pip install -e '.[chart]'"
        )
    save_result_chart(
...
def main():
    ...
    save_result_csv(sorted_skills, settings.csv_output)
